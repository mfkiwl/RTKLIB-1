...
static int read_raw_data_stream(FILE *fp, stream_t *stream, unsigned char *buff,
                                int n) {
  if (!stream->port) {
...
static int decode_rxmrawx(raw_t *raw) {
  ...
  int i, j, k, f, sys, prn, sat, code, slip, halfv, halfc, LLI, n = 0,
                                                                std_slip = 0;
  int week, nmeas, ver, gnss, svid, sigid, frqid, lockt, cn0, cpstd, prstd,
      tstat;
...
static int decode_trkmeas(raw_t *raw) {
  ...
  const char P_adj_fw2[] = {0,  0,  0,  0,  1,  3,  2, 0,
                            -4, -3, -9, -8, -7, -4, 0}; /* fw 2.30 */
  const char P_adj_fw3[] = {11, 13, 13, 14, 14, 13, 12, 10,
                            8,  6,  5,  5,  5,  7,  0}; /* fw 3.01 */
...
static int decode_rxmsfrbx(raw_t *raw) {
  ...
  channel = U1(p + 5);
  version = U1(p + 6);
  // unsigned int reserved2 = U1(p + 7);

  // Use the signal ID if defined
...
static int read_service_chars(raw_t *const raw, FILE *fp, stream_t *stream) {
  ...
  int bytes_read =
      read_raw_data_stream(fp, stream, raw->buff + raw->nbyte, bytes_to_read);
  ...
  /* raw->len consists of payload length, sync chars, class, ID, length and
   * checksum fields */
  ...
  if (raw->len > MAXRAWLEN) {
    trace(2, "ubx length error: len=%d\n", raw->len);
    raw->nbyte =
        0; /* reset nbyte to enter synchronize frame in next iteration */
...
static int read_payload(raw_t *const raw, FILE *fp, stream_t *stream) {
  ...
  payload_read_length = read_raw_data_stream(fp, stream, raw->buff + raw->nbyte,
                                             raw->len - raw->nbyte);

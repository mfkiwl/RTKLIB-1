...
static void detslp_ll(rtk_t *rtk, const obsd_t *obs, int n)
{
    ...
    for (i=0;i<n&&i<MAXOBS;i++) for (j=0;j<nf;j++) {
        ...
        trace(3,"detslp_ll: slip detected sat=%s f=%d\n",satno2id_str(obs[i].sat),j+1);
...
static void detslp_gf(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        ...
        trace(4,"detslip_gf: sat=%s gf0=%8.3f gf1=%8.3f\n",satno2id_str(obs[i].sat),g0,g1);

        if (g0!=0.0&&fabs(g1-g0)>rtk->opt.thresslip) {
            trace(3,"detslip_gf: slip detected sat=%s gf=%8.3f->%8.3f\n",
                  satno2id_str(obs[i].sat),g0,g1);
...
static void detslp_mw(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        ...
        trace(4,"detslip_mw: sat=%s mw0=%8.3f mw1=%8.3f\n",satno2id_str(obs[i].sat),w0,w1);

        if (w0!=0.0&&fabs(w1-w0)>THRES_MW_JUMP) {
            trace(3,"detslip_mw: slip detected sat=%s mw=%8.3f->%8.3f\n",
                  satno2id_str(obs[i].sat),w0,w1);
...
static void udbias_ppp(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    for (f=0;f<NF(&rtk->opt);f++) {
        ...
        for (i=0;i<n&&i<MAXOBS;i++) {
            ...
            trace(5,"udbias_ppp: sat=%s bias=%.3f\n",satno2id_str(sat),bias[i]);
        }
    }
}
                   ...
                   double *azel)
{
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        ...
        for (j=0;j<2*NF(opt);j++) {
            ...
            trace(4,"%s sat=%s %s%d res=%9.4f sig=%9.4f el=%4.1f\n",str,satno2id_str(sat),
            ...
            if (!post&&opt->maxinno>0.0&&fabs(v[nv])>opt->maxinno) {
                trace(2,"outlier (%d) rejected %s sat=%s %s%d res=%9.4f el=%4.1f\n",
                      post,str,satno2id_str(sat),j%2?"P":"L",j/2+1,v[nv],azel[1+i*2]*R2D);
    ...
    if (post&&ne>0) {
        ...
        sat=obs[maxobs].sat;
        trace(2,"outlier (%d) rejected %s sat=%s %s%d res=%9.4f el=%4.1f\n",
            post,str,satno2id_str(sat),maxfrq%2?"P":"L",maxfrq/2+1,vmax,azel[1+maxobs*2]*R2D);

...
extern void alm2pos(gtime_t time, const alm_t *alm, double *rs, double *dts)
{
    ...
    trace(4,"alm2pos : time=%s sat=%s\n",time_str(time,3),satno2id_str(alm->sat));
    ...
    if (n>=MAX_ITER_KEPLER) {
        trace(2,"alm2pos: kepler iteration overflow sat=%s\n",satno2id_str(alm->sat));
        return;
    }
...
extern double eph2clk(gtime_t time, const eph_t *eph)
{
    ...
    trace(4,"eph2clk : time=%s sat=%s\n",time_str(time,3),satno2id_str(eph->sat));
                    ...
                    double *var)
{
    ...
    if (n>=MAX_ITER_KEPLER) {
        trace(2,"eph2pos: kepler iteration overflow sat=%s\n",satno2id_str(eph->sat));
        return;
    }
    sinE=sin(E); cosE=cos(E);

    trace(4,"kepler: sat=%s e=%8.5f n=%2d del=%10.3e\n",satno2id_str(eph->sat),eph->e,n,E-Ek);
...
extern double geph2clk(gtime_t time, const geph_t *geph)
{
    ...
    trace(4,"geph2clk: time=%s sat=%s\n",time_str(time,3),satno2id_str(geph->sat));
...
extern double seph2clk(gtime_t time, const seph_t *seph)
{
    ...
    trace(4,"seph2clk: time=%s sat=%s\n",time_str(time,3),satno2id_str(seph->sat));
...
static eph_t *seleph(gtime_t time, int sat, int iode, const nav_t *nav)
{
    ...
    trace(4,"seleph  : time=%s sat=%s iode=%d\n",time_str(time,3),satno2id_str(sat),iode);
    ...
    if (iode>=0||j<0) {
        trace(3, "no broadcast ephemeris: %s sat=%s iode=%3d\n", time_str(time, 0),
              satno2id_str(sat), iode);
        return NULL;
    }
    return nav->eph+j;
}
...
static geph_t *selgeph(gtime_t time, int sat, int iode, const nav_t *nav)
{
    ...
    trace(4,"selgeph : time=%s sat=%s iode=%2d\n",time_str(time,3),satno2id_str(sat),iode);
    ...
    if (iode>=0||j<0) {
        trace(3, "no glonass ephemeris  : %s sat=%s iode=%2d\n", time_str(time, 0),
              satno2id_str(sat), iode);
        return NULL;
    }
    return nav->geph+j;
...
static seph_t *selseph(gtime_t time, int sat, const nav_t *nav)
{
    ...
    trace(4, "selseph : time=%s sat=%s\n", time_str(time, 3), satno2id_str(sat));
    ...
    if (j<0) {
        trace(3, "no sbas ephemeris     : %s sat=%2f\n", time_str(time, 0), satno2id_str(sat));
        return NULL;
    }
    return nav->seph+j;
}
                  ...
                  double *dts)
{
    ...
    seph_t *seph;
    int sys;
    char *id =satno2id_str(sat);
    trace(4, "ephclk  : time=%s sat=%s\n", time_str(time, 3), satno2id_str(sat));
                  ...
                  int iode, double *rs, double *dts, double *var, int *svh)
{
    ...
    trace(4, "ephpos  : time=%s sat=%s iode=%d\n", time_str(time, 3), satno2id_str(sat), iode);
                        ...
                        double *rs, double *dts, double *var, int *svh)
{
    ...
    trace(4, "satpos_sbas: time=%s sat=%s\n", time_str(time, 3), satno2id_str(sat));
    ...
    if (i>=nav->sbssat.nsat) {
        trace(2, "no sbas correction for orbit: %s sat=%s\n", time_str(time, 0), satno2id_str(sat));
                      ...
                      int opt, double *rs, double *dts, double *var, int *svh)
{
    ...
    trace(4, "satpos_ssr: time=%s sat=%s\n", time_str(time, 3), satno2id_str(sat));

    ssr=nav->ssr+sat-1;

    if (!ssr->t0[0].time) {
        trace(2,"no ssr orbit correction: %s sat=%s\n",time_str(time,0),satno2id_str(sat));
        return 0;
    }
    if (!ssr->t0[1].time) {
        trace(2,"no ssr clock correction: %s sat=%s\n",time_str(time,0),satno2id_str(sat));
        return 0;
    }
    /* inconsistency between orbit and clock correction */
    if (ssr->iod[0]!=ssr->iod[1]) {
        trace(2,"inconsist ssr correction: %s sat=%s iod=%d %d\n",
              time_str(time,0),satno2id_str(sat),ssr->iod[0],ssr->iod[1]);
        *svh=-1;
        return 0;
    }
    ...
    if (fabs(t1)>MAXAGESSR||fabs(t2)>MAXAGESSR) {
        trace(2,"age of ssr error: %s sat=%s t=%.0f %.0f\n",time_str(time,0),
              satno2id_str(sat),t1,t2);
        *svh=-1;
        return 0;
    }
    ...
    *var=var_urassr(ssr->ura);
    ...
    trace(5,"satpos_ssr: %s sat=%s deph=%6.3f %6.3f %6.3f er=%6.3f %6.3f %6.3f dclk=%6.3f var=%6.3f\n",
          time_str(time,2),satno2id_str(sat),deph[0],deph[1],deph[2],er[0],er[1],er[2],dclk,*var);
...
extern int satpos(gtime_t time, gtime_t teph, int sat, int ephopt,
                  ...
                  int *svh)
{
    trace(4,"satpos  : time=%s sat=%s ephopt=%d\n",time_str(time,3),satno2id_str(sat),ephopt);
                    ...
                    int ephopt, double *rs, double *dts, double *var, int *svh)
{
    ...
    for (i=0;i<n&&i<2*MAXOBS;i++) {
        ...
        if (j>=NFREQ) {
            trace(2,"no pseudorange %s sat=%s\n",time_str(obs[i].time,3),satno2id_str(obs[i].sat));
            continue;
        }
        ...
        if (!ephclk(time[i],teph,obs[i].sat,nav,&dt)) {
            trace(3,"no broadcast clock %s sat=%s\n",time_str(time[i],3),satno2id_str(obs[i].sat));
            continue;
        }
    ...
    for (i=0;i<n&&i<2*MAXOBS;i++) {
        trace(4,"%s sat=%s rs=%13.3f %13.3f %13.3f dts=%12.3f var=%7.3f svh=%02X\n",

    ...
    *var=0.0;
    ...
    if (iter>0) {
        if (testsnr(0,i,azel[1],obs->SNR[i]*0.25,&opt->snrmask)) {
            trace(4,"snr mask: %s sat=%s el=%.1f snr=%.1f\n",
                  time_str(obs->time,0),satno2id_str(obs->sat),azel[1]*R2D,obs->SNR[i]*0.25);
            return 0.0;
        }
                   ...
                   double *azel, int *vsat, double *resp, int *ns)
{
    ...
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
        ...
        if (i<n-1&&i<MAXOBS-1&&obs[i].sat==obs[i+1].sat) {
            trace(2,"duplicated observation data %s sat=%s\n",
        ...
        trace(4,"sat=%s azel=%5.1f %4.1f res=%7.3f sig=%5.3f\n",satno2id_str(obs[i].sat),
                    ...
                    sol_t *sol, double *azel, int *vsat, double *resp, char *msg)
{
    ...
    for (i=0;i<n;i++) {
        ...
        if (!estpos(obs_e,n-1,smoothing_data,rs_e,dts_e,vare_e,svh_e,nav,opt,&sol_e,
            ...
            trace(3,"raim_fde: exsat=%s (%s)\n",satno2id_str(obs[i].sat),msg);
            continue;
        }
        ...
        if (nvsat<5) {
            trace(3,"raim_fde: exsat=%s lack of satellites nvsat=%2d\n",
                  satno2id_str(obs[i].sat),nvsat);
            continue;
        }
        rms_e=sqrt(rms_e/nvsat);

        trace(3,"raim_fde: exsat=%s rms=%8.3f\n",satno2id_str(obs[i].sat),rms_e);
